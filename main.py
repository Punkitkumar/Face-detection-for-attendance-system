...
class App:
    def __init__(self):
        # section 1 : designing our webcam and integrating it with util.py#
    ...
    def add_webcam(self, label):
        if 'cap' not in self.__dict__:
            self.cap = cv2.VideoCapture(0)  ##here we are changing VideCapture format (frames) from cv2 to pillow format
    ...
    def process_webcam(self):
        ...
        self._label.configure(image=imgtk)  # upto this we are making only one frame

        self._label.after(20, self.process_webcam)  # take after every 20ms capture
    ...
    def register_new_user(self):
        ...
        self.try_again_accept_button_register_new_user_window = util.get_button(self.register_new_user_window,
                                                                                'Try again', 'red',
                                                                                self.try_again_register_new_user)
        self.try_again_accept_button_register_new_user_window.place(x=750, y=400)
